  ...
  OpenLayers.Control.Panel, {
    ...
    initialize: function(layer, options) {
        ...
        if (options.allow_edit && options.allow_edit !== 0) {
          ...
          controls = [new OpenLayers.Control.Navigation({'title' : title})];
        }
        ...
        if (options.feature_types && options.feature_types.length) {
          for (var i = 0, il = options.feature_types.length; i < il; i += 1) {
            ...
            var title = Drupal.t('Draw ' + tool);
            controls.push(
              new OpenLayers.Control.DrawFeature(layer, OpenLayers.Handler[tool], {
                  'title' : title,
                  'displayClass': 'olControlDrawFeature' + tool
              })
            );
          }
        }
  ...
  Drupal.behaviors.openlayers_behavior_geofield = {
    'attach': function(context, settings) {
      ...
      if (!$(context).hasClass('geofield-processed')) {
        ...
        if ($wkt.length) {
            var dataLayers = data.openlayers.getLayersBy('drupalID', "geofield_formatter");
            // If the Geofield Layer is not selected, we just create one on the fly
            if (dataLayers.length == 0) {
              var dataLayer = new OpenLayers.Layer.Vector(
                  Drupal.t("Geofield Input Layer"),
                  {
                    projection: dataProjection,
                    drupalID: 'geofield_formatter'
                  }
              );
              // We add the default styles to the layer, because we have nothing better
              var styleMap = Drupal.openlayers.getStyleMap(data.map, 'geofield_formatter');
              dataLayer.StyleMap = styleMap;
              data.openlayers.addLayer(dataLayer);
            }
            else {
                dataLayer = dataLayers[0];      
            }
